...
class ExcelToSQL {
    ...
    private $stpi_id = "";       // 專利平台匯入資料用
    private $lawsuit_id = "";   // 專利平台匯入資料用
    private $lawsuit_name = ""; // 專利平台匯入資料用
    ...
    // 專利平台匯入資料用
    public function setSTPI_ID($id) {
        $this->stpi_id = $id;
    }

    public function setLawsuitID($id) {
        $this->lawsuit_id = $id;
    }

    public function setLawsuitName($name) {
        $this->lawsuit_name = $name;
    }
    ...
    public function handleExcelFile($rowIndex=2) {
        if($this->file == "") return 0;

        $tempDB = new TempDB($this->dbh);
        $filename = explode('/', $this->file);
        $filename = $filename[count($filename)-1];
        // echo "filename = ".$filename."<br>";

        // $lawsuit_id = substr($filename, 0, strpos($filename, '_'));
        // echo "lawsuit_id = ".$lawsuit_id."<br>";

        //資料筆數
        $rCount = 0;

        $highestRow = $this->activeSheet->getHighestRow();
        echo ", 分頁名稱 [".$this->sheetName."], 最高行數 = ".$highestRow;
        // 手動設定讀取範圍
        // if($highestRow > 12)
        //     $highestRow = 12;
        echo "(讀取".$highestRow."筆資料)";

        for($j = $rowIndex ; $j <= $highestRow; $j++){
        // for($j = $rowIndex ; $j <= $rowIndex; $j++){
            $start = microtime(true);
            $this->skeep = false;
            // echo $this->activeSheet->getCell("B"."$j")->getValue().", ";

            $tempObj = new stdClass;
            $tempObj->stpi_id     = $this->stpi_id;
            // $tempObj->lawsuit_id  = $this->lawsuit_id;
            // $tempObj->lawsuit_name= $this->lawsuit_name;
            $tempObj->level       = $this->activeSheet->getCell("A"."$j")->getValue();
            $tempObj->root_node   = $this->activeSheet->getCell("B"."$j")->getValue();
            $tempObj->child_node  = $this->activeSheet->getCell("C"."$j")->getValue();
            $tempObj->is_link     = $this->activeSheet->getCell("D"."$j")->getValue();
            $tempObj->content     = $this->activeSheet->getCell("E"."$j")->getValue();

            // 訴訟報告特別處理
            if ($tempObj->level == "1" &&
                $tempObj->child_node == "評析及衍生議題解析") {
                $tempObj->root_node = $tempObj->child_node;
            }

            // 技術報告特別處理
            if ($tempObj->level == "1" && (
                $tempObj->child_node == "結論" ||
                $tempObj->child_node == "比對結果" ||
                $tempObj->child_node == "本專利與標準文意相符的章節與段落")) {
                $tempObj->root_node = $tempObj->child_node;
                $tempObj->child_node = $tempObj->content;
                $tempObj->content = null;
            }


            if ($tempObj->root_node != "") {
                $tempDB->insert($tempObj);
                $rCount++;
            }
